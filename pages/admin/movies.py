...
class AdminMovies:
    def __init__(self):
        ...
        self.title = Label(self.window, text='Movies', font=('Fredoka', 25, "bold"), pady=20, fg="#d3aa1d",
                           bg="#35374f")
        self.title.grid(row=0, column=1, columnspan=10, sticky="W")

        # Treeview that will show th movies
        self.treeview = ttk.Treeview(self.window)
        self.treeview.grid(row=1, column=1, columnspan=10, rowspan=6, sticky="WE")
        self.treeview['columns'] = ('ID', 'Title', 'Stock', 'Overview', 'Release Date', 'Genres', 'Director', 'Rating',
                                    'Price Month', 'Price')
        ...
        # This will show which page the user is in
    ...
    def get_and_show_movies(self):
        ...
        # Get 25 movies per page, offset by page number
        movies = MovieModel().get_instance().get_movie_count(25, self.page * 25)
        movie_count = MovieModel().get_instance().get_movie_count_number()

        self.lblPage.config(
            text=f"Page: {str(self.page)}, Showing 25 movies per page ({str(movie_count)} movies in total)")
        for movie in list(movies):  # Update the treeview
            self.treeview.insert('', 'end', text='', values=(movie[0], movie[1], movie[9], movie[2],
                                                             movie[3], movie[4], movie[5], movie[6], movie[7],
                                                             movie[8]))

    @staticmethod
    def show_genre_stats():
        movies = MovieModel().get_instance().get_all_movies()  # Get all the movies

        # Loop through the movies and count the genres
        genres = {}
        for movie in movies:
    ...
    def next_page(self):
        movie_count = MovieModel().get_instance().get_movie_count_number()  # Get the total number of movies
        self.page += 1
        self.get_and_show_movies()
        self.lblPage.config(  # Update the page label
            text=f"Page: {str(self.page + 1)}, Showing 25 movies per page ({str(movie_count)} movies in total)")
    ...
    def find_movie(self):
        ...
        # Update the treeview with the found movies
    ...
    def update_movie(self):
        selected_item = self.treeview.selection()  # Get the selected movie
        ...
        if (not name or
                ...
                not director or
                not rating or
                not price or not stock):
            messagebox.showerror('Error', 'All fields are required')
            return

        # Update the movie
        MovieModel().get_instance().update_movie(movie_id, name, overview, release_date, genres, director, rating,
                                                 price_month, price, stock)
        self.get_and_show_movies()
    ...
    def create_movie(self):
        ...
        # Create the movie
        MovieModel().get_instance().create_movie(name, overview, release_date, genres, director, rating, price_month,
                                                 price, stock)
        self.get_and_show_movies()
